...
function goToAnotherPage(max, recordsPerPage, moduleName, action, id)
{
	var page = document.getElementById("pageToGo").value * 1;
	if (!isNaN(page) && page <= max && page > 0) {
		var path = "/" + moduleName + "/" + action + "/" + ((page - 1) * recordsPerPage);
		
		if (id != -1) {
			path += "/" + id;
		}
		document.location = '${JForumContext.encodeURL("' + path + '")}';
	}
}
