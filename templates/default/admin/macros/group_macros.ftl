	...
	<#if (total >= 0 )>
		<#list 0..total as i>
			...
			<tr bgcolor="#f4f4f4">
				...
				<td align="center"><span class="gen"><a href="${JForumContext.encodeURL("/${moduleName}/edit/${node.id}")}">${I18n.getMessage("Groups.List.Edit")}</a></span></td>
				<td align="center"><input type="checkbox" name="group_id" value="${node.id}"/></td>
				<td class="row2" align="center"><span class="gen"><a href="${JForumContext.encodeURL("/${moduleName}/permissions/${node.id}")}">${I18n.getMessage("Permissions")}</a></span></td>
			</tr>
