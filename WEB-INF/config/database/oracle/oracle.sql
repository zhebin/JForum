...
# UserModel
# ##########
UserModel.addNew = INSERT INTO jforum_users (user_id, username, user_password, user_email, user_regdate, user_actkey, user_msnm, user_icq, rank_id) VALUES (jforum_users_seq.nextval, ?, ?, ?, ?, ?, ?, ?, 0)

UserModel.selectAllByLimit = SELECT * FROM ( \
        SELECT user_email, user_id, user_posts, user_regdate, username, deleted, user_karma, user_from, user_website, user_msnm, user_viewemail, ROW_NUMBER() OVER(ORDER BY username) - 1 LINENUM \
...
TopicModel.selectAllByForumByLimit = SELECT * \
  				FROM (SELECT t.*, u1.user_msnm as last_user_name, u2.user_msnm as post_by_name, p.user_id AS last_user_id, p.post_time, \
               	(SELECT SUM(p.attach) FROM jforum_posts p WHERE p.topic_id = t.topic_id AND p.need_moderate = 0) AS attach, \
...
TopicModel.selectByUserByLimit = SELECT * FROM ( \
    SELECT t.*, u1.user_msnm as last_user_name, u2.user_msnm as post_by_name, p.user_id AS last_user_id, p.post_time, (SELECT SUM(p.attach) \
        FROM jforum_posts p \
...
TopicModel.selectRecentTopicsByLimit = SELECT * FROM ( \
       SELECT t.*, p.user_id AS last_user_id, p.post_time, p.attach AS attach, u1.user_msnm as last_user_name, u2.user_msnm as post_by_name, \
       ROW_NUMBER() OVER(ORDER BY topic_type DESC, topic_time DESC) - 1 LINENUM \
       FROM jforum_topics t, jforum_posts p, jforum_users u1, jforum_users u2 \
       WHERE p.post_id = t.topic_last_post_id \
       AND p.need_moderate = 0 \
       AND u1.user_id = p.user_id \
...
TopicModel.selectHottestTopicsByLimit = SELECT * FROM (\
	SELECT t.*, p.user_id AS last_user_id, p.post_time, p.attach AS attach, u1.user_msnm as last_user_name, u2.user_msnm as post_by_name, \
	ROW_NUMBER() OVER(ORDER BY topic_replies DESC, topic_last_post_id DESC) - 1 LINENUM \
    FROM jforum_topics t, jforum_posts p, jforum_users u1, jforum_users u2 \
    WHERE p.post_id = t.topic_last_post_id \
    AND p.need_moderate = 0 \
    AND t.topic_replies >= ? \
...
# SearchModel
# ############
SearchModel.firstPostIdByDate = SELECT * FROM (\
	SELECT post_id, ROW_NUMBER() OVER(ORDER BY post_id) -1 LINENUM \
	FROM jforum_posts WHERE post_time > ? ORDER BY post_id) \
	WHERE LINENUM = 0
