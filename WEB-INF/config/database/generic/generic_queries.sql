...
# UserOwnGroup
# #############
UserOwnGroup.selectById = SELECT * FROM jforum_user_owngroup WHERE group_id = ?
UserOwnGroup.selectByUserId = SELECT * FROM jforum_user_owngroup WHERE user_id = ? order by create_time desc
UserOwnGroup.update = UPDATE jforum_user_owngroup SET group_name = ? WHERE group_id = ?
UserOwnGroup.addNew =  INSERT INTO jforum_user_owngroup (group_id, group_name, create_time, user_id) VALUES (JFORUM_USER_OWNGROUP_SEQ.nextval, ?, ?, ? )
UserOwnGroup.delete = DELETE FROM jforum_user_owngroup WHERE group_id = ?
UserOwnGroup.lastGeneratedId = SELECT JFORUM_USER_OWNGROUP_SEQ.Currval from dual

# #############
# UserOwnGroupUser
# #############
UserOwnGroupUser.selectUserByGroupId = SELECT user_id,user_msnm FROM jforum_users WHERE USER_ID in (SELECT user_id FROM jforum_user_owngroup_user WHERE group_id = ?)
UserOwnGroupUser.addNew = INSERT INTO jforum_user_owngroup_user ( group_id, user_id) VALUES ( ?, ? )
UserOwnGroupUser.delete = DELETE FROM jforum_user_owngroup_user WHERE group_id = ? and user_id= ?
UserOwnGroupUser.find = SELECT group_id,user_id FROM jforum_user_owngroup_user WHERE group_id = ? and user_id= ?
...
UserModel.selectAllByGroup = SELECT user_email, u.user_id, user_posts, user_regdate, username, deleted, user_karma, user_from, \
	user_website, user_viewemaill,user_msnm \
...
UserModel.addNew = INSERT INTO jforum_users (username, user_password, user_email, user_regdate, user_actkey, user_msnm, user_icq, rank_id) VALUES (?, ?, ?, ?, ?, ?, ?, 0)
UserModel.findByName = SELECT user_id, username, user_email, deleted, user_msnm FROM jforum_users WHERE UPPER(user_msnm) LIKE UPPER(?) or UPPER(username) LIKE UPPER(?)
...
ForumModel.getTodayTotal = select count(*) as postTotal from jforum_posts where forum_id = ? and to_char(post_time,'yyyy-mm-dd') = to_char(sysdate,'yyyy-mm-dd') \

ForumModel.getTotalPosts = select count(*) as postTotal from jforum_posts where forum_id = ? \
...
ForumModel.lastPostInfo = SELECT post_time, p.topic_id, t.topic_replies, post_id, u.user_id, u.user_msnm as name \
...
ForumModel.getModeratorList = SELECT u.user_id AS id, u.user_msnm AS name \
...
TopicModel.selectById = SELECT t.*, p.user_id AS last_user_id, p.post_time, p.attach AS attach, u1.user_msnm as last_user_name, u2.user_msnm as post_by_name \
	FROM jforum_topics t, jforum_posts p, jforum_users u1, jforum_users u2 \
	WHERE t.topic_id = ? \
	AND p.post_id = t.topic_last_post_id \
	AND u1.user_id = p.user_id \
...
SearchModel.getPostsDataForLucene = SELECT p.post_id, p.forum_id, p.topic_id, p.user_id, u.user_msnm, p.enable_bbcode, p.enable_smilies, p.post_time, pt.post_subject, pt.post_text, t.topic_title \
...
PrivateMessageModel.baseListing = SELECT pm.privmsgs_type, pm.privmsgs_id, pm.privmsgs_date, pm.privmsgs_subject, u.user_id, u.username, u.user_msnm \
...
ModerationModel.topicsByForum = SELECT p.post_id, t.topic_id, t.topic_title, t.topic_replies, p.user_id, enable_bbcode, p.attach, \
	enable_html, enable_smilies, pt.post_subject, pt.post_text, user_msnm \
	FROM jforum_posts p, jforum_posts_text pt, jforum_users u, jforum_topics t \
	WHERE p.post_id = pt.post_id \
	AND p.topic_id = t.topic_id \
	AND t.forum_id = ? \
...
TopicModel.stickyNotSticky = UPDATE jforum_topics SET topic_type = ? WHERE topic_id = ? and topic_type != 2